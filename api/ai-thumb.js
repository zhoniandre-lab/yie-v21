/**
 * POST /api/ai-thumb
 * Generate thumbnail IMAGE via IAMHC (OpenAI-compatible /images/generations)
 * FAIL-FAST: 1 request per call (no server sleep) → avoids Vercel HTTP 504
...
function detectIntent(seed = '', angle = '', title = '', niche = '') {
  ...
  if (/(cerita jadi lagu|lagu sedih|lirik|story_song)/.test(t)) {
    return {
      id: 'story_song',
      label: 'Cerita jadi lagu / emosional',
      searcher_goal:
      ...
      avoid: 'DJ neon club, horror ghost, funny meme, random off-topic',
    };
  }
  if (/(sebelum.*lahir|lahir tanpa|pergi sebelum|meninggal sebelum)/.test(t)) {
    return {
      ...
      searcher_goal:
        'Orang mencari cerita/lagu emosional tentang ibu yang tiada sebelum anak lahir',
      visual:
  ...
  return [
    ...
    patterns && patterns.length
      ? `Title patterns mined: ${patterns
          ...
          .join(', ')}`
      : '',
    userPrompt ? `Creator / prompt box direction: ${userPrompt}` : '',
...
async function fetchUrlAsBase64(imageUrl, apiKey) {
  const headerVariants = [
    {
      'User-Agent': 'Mozilla/5.0 (compatible; YIE/21.15)',
      Accept: 'image/*,*/*',
    },
    {
      'User-Agent': 'Mozilla/5.0 (compatible; YIE/21.15)',
...
/**
 * FAIL-FAST single request. No server-side sleep (sleep causes Vercel 504).
 */
async function generateImage({ prompt, model, baseUrl, apiKey, size = '1024x1024' }) {
  ...
  try {
    ...
    if (!b64 && imageUrl) {
      try {
        ...
        return {
          ...
          raw: data,
          model,
          warning: 'URL image; proxy base64 gagal.',
        };
      }
    }
    ...
    if (e && e.name === 'AbortError') {
      const err = new Error('TIMEOUT_IMAGE: provider terlalu lama (>22s)');
      err.timeout = true;
      throw err;
    }
    throw e;
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (
      String(body.contentFormat || body.nicheNote || '').match(
        /story_song|cerita jadi lagu|lagu sedih|lirik/i
      )
    ) {
      intent = {
        id: 'story_song',
    ...
    // Prefer explicit client prompt box when long enough (user-edited / AI-polished)
    const finalPrompt = clientPrompt.length > 40 ? clientPrompt : basePrompt;
    ...
    return res.status(200).json({
      ...
      result: {
        ...
        ctr_reason: 'On-intent high-CTR composition from engine prompt.',
    ...
    const concurrency = e.concurrency || isConcurrencyError(msg);
    const timeout = e.timeout || /TIMEOUT_IMAGE|aborted|timeout/i.test(msg);
    return res.status(timeout ? 504 : 502).json({
      ok: false,
      error: msg,
      code: concurrency ? 'CONCURRENCY' : timeout ? 'TIMEOUT' : 'IMAGE_FAIL',
      retryable: !!(concurrency || timeout),
      retryAfterSec: concurrency ? 7 : timeout ? 4 : 0,
      hint: concurrency
        ? 'Slot image penuh. Client akan coba lagi otomatis (cepat).'
        : timeout
          ? 'Provider lambat. Client retry singkat.'
          : 'Cek IAMHC_IMAGE_MODEL=step-image-edit-2',
    });
  }
};
